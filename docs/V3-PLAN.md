...
- **실서버 검증 미실시** — V3 계정(운영자 발급 `loginId`)이 아직 없다.

## 6. 남은 확인 항목

| # | 내용 | 근거 |
|---|---|---|
| Q1 | V3 테스트 계정(또는 `X-Admin-Key`) — 실서버 통합 검증에 필요 | 관리자 API로만 발급 |
...
| Q4 | 앱 종료 시 저장 — `beforeunload`의 비동기 요청은 보장되지 않는다. Tauri 창 닫기 이벤트를 가로채는 편이 확실하다 | 탭별 작업물 보존 D-2 |
| Q5 | V3.1 관리자 페이지(계정 발급·사용량·운영)는 이번 범위 밖 | 기능정의서 개발단계 V3.1 |
