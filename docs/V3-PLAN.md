...
## 5. 확인이 필요한 항목 (진행하며 질의)

| # | 내용 | 근거 |
|---|---|---|
| Q1 | `PUT .../elements` 등 5개 미구현 API의 BE 배포 예정일 — M2/M5 완료 시점이 여기에 묶임 | API 명세 "시작 전" |
| Q2 | 중복 로그인으로 밀려난 세션의 FE 감지 시점 — 401/AUTH4003 수신 시로 충분한가 (별도 푸시 없음) | 로그인 D-6 미결 |
| Q3 | 소셜 로그인 코드 완전 삭제 vs 보존 (B2C 재도입 대비) | 소셜 로그인 D-4 미결 |
| Q4 | 파일 1,000개 상한 경고 UI의 트리거(응답 필드 없음 — BE 확인 필요) | 마이페이지 D-2 |
