...
> ⚠️ **2026-07-27 재검증됨.** 아래 §4(“대응 API가 없어 서버 저장 불가”)는 **더 이상 유효하지 않다** —
> 블록 추가/삭제/순서변경 API가 명세에 추가되어 FE 연동을 마쳤다. 최신 상태는
> [API-VERIFICATION-2026-07-27.md](./API-VERIFICATION-2026-07-27.md) 참고.
...
## 4. ~~FE 한계 — 명세에 대응 API가 없어 서버 저장이 불가한 기능~~ (2026-07-27 해소)

> 이 절은 2026-07-06 시점 기준이다. **2026-07-27 재검증에서 세 API가 모두 명세에 추가되고 실서버에서
> 동작함을 확인해 FE 연동을 마쳤다.** 아래 표는 이력용으로 남긴다.

| 에디터 기능 | 대응 API | 2026-07-06 | 2026-07-27 |
| --- | --- | --- | --- |
| 블록 새로 추가 (`+` 버튼) | `POST .../pages/{pageNo}/elements` | ❌ 로컬 전용 | ✅ 연동 완료 |
| 블록 삭제 | `DELETE .../elements/{elementId}?elementType=` | ❌ 로컬 전용 | ✅ 연동 완료 |
| 블록 순서 변경 (드래그) | `PATCH .../elements/order` | ❌ 로컬 전용 | ✅ 연동 완료 |
| 블록 내용 수정 / 초안 선택 | `PATCH .../elements/{elementId}` | ✅ 연동 완료 | ✅ |
